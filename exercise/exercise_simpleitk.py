...
def register_images(img, label_img, atlas_img):
    ...
    registered_img = sitk.Resample(atlas_img, transform, sitk.sitkLinear)  # todo: modify here
    ...
    registered_label = sitk.Resample(label_img, atlas_img, transform, sitk.sitkNearestNeighbor)  # todo: modify here
...
def preprocess_rescale_numpy(np_img, new_min_val, new_max_val):
    ...
    factor = (new_max_val - new_min_val) / (max_val - min_val)
    rescaled_np_img = ((np_img - min_val) * factor) + new_min_val  # todo: modify here
